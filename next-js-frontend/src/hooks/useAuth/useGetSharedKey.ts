...
export const useGetSharedKey = () => {
  const getSharedKey = async ({
    ...
    const existingSharedKeyInJwkFormat = await getStoredSharedKeyFromIndexedDB({
      ...
      userId2: otherMember._id,
    });

    if (existingSharedKeyInJwkFormat) {
      // if it is found, we convert it to crypto format and return it
      const sharedSecretKeyInCryptoFormat = await convertJwkToCryptoKey({
        isPrivateKey: false,
        KeyInJwkFormat: existingSharedKeyInJwkFormat,
      });
      ...
      const userPrivateKeyInJwkFormat = await getUserPrivateKeyFromIndexedDB({
      ...
      if (userPrivateKeyInJwkFormat && otherMemberPublicKeyInJwkFormat) {
        ...
        const [
          userPrivateKeyInCryptoFormat,
          otherMemberPublicKeyInCryptoFormat,
        ] = await Promise.all([
          convertJwkToCryptoKey({
            isPrivateKey: true,
            KeyInJwkFormat: userPrivateKeyInJwkFormat,
          }),
          await convertJwkToCryptoKey({
            isPrivateKey: false,
            KeyInJwkFormat: JSON.parse(otherMember.publicKey),
          }),
        ]);
        ...
        if (
          ...
          const sharedSecretKeyInCrytoFormat = await deriveSharedSecretKey({
            privateKey: userPrivateKeyInCryptoFormat,
            publicKey: otherMemberPublicKeyInCryptoFormat,
          });
          ...
          if (sharedSecretKeyInCrytoFormat) {
            ...
            const sharedSecretKeyInJwkFormat = await convertCryptoKeyToJwk({
              cryptoKey: sharedSecretKeyInCrytoFormat,
            });
            if (sharedSecretKeyInJwkFormat) {
              storeSharedKeyInIndexedDB({
                sharedKeyJwk: sharedSecretKeyInJwkFormat,
                userId1: loggedInUserId,
                userId2: otherMember._id,
              });
            }
