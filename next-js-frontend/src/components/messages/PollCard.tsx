...
export const PollCard = ({question,options,isMutipleAnswers,messageId}:PropTypes) => {

  const loggedInUser = useAppSelector(selectLoggedInUser) as User
  const {handleViewVotesClick} = useViewVotesClick({pollQuestion:question,pollOptions:options});
