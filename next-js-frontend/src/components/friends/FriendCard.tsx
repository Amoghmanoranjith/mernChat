...
export const FriendCard = ({friend,selectedMembers,handleAddOrRemoveMember}:PropTypes) => {
  return (
    <motion.div whileHover={{x:-1}} whileTap={{scale:0.980}} onClick={()=>handleAddOrRemoveMember(friend._id)} className={`flex items-center gap-x-2 ${selectedMembers.includes(friend._id)?"bg-primary text-white hover:bg-primary-dark shadow-2xl":"bg-secondary text-text hover:bg-secondary-darker"} p-2 rounded-lg cursor-pointer`}>
